...
// ============================================================
// === MÓDULO BI (VISÃO DE ÁGUIA EXPANDIDA) ===
// ============================================================

let chartExpandido = null;
let contextoAtualBI = ''; // 'financeiro' ou 'vendas'

// 1. Abre o Modal e Carrega Preferências
window.abrirGraficoBI = function(tipo) {
    ...
    // Define datas padrão (Mês atual completo) se estiverem vazias
    const hoje = new Date();
    const inicioMes = new Date(hoje.getFullYear(), hoje.getMonth(), 1).toISOString().split('T')[0];
    const fimMes = new Date(hoje.getFullYear(), hoje.getMonth() + 1, 0).toISOString().split('T')[0];
    
    if(!document.getElementById('biDataInicio').value) document.getElementById('biDataInicio').value = inicioMes;
    if(!document.getElementById('biDataFim').value) document.getElementById('biDataFim').value = fimMes;

    // Recupera a preferência salva no computador (LocalStorage)
    const prefSalva = localStorage.getItem(`pref_grafico_${tipo}`);
    if (prefSalva) {
        selectTipo.value = prefSalva;
    } else {
        // Padrões se nunca salvou
        selectTipo.value = (tipo === 'financeiro') ? 'doughnut' : 'bar';
    }

    if (tipo === 'financeiro') titulo.innerText = "💰 Análise Financeira (Despesas vs Gastos)";
    else titulo.innerText = "🏆 Performance de Vendas (Ranking)";

    modal.style.display = 'flex';
    window.filtrarGraficoExpandido(); // Gera o gráfico
}

// 2. Função que Recalcula TUDO baseado nas datas escolhidas
window.filtrarGraficoExpandido = function() {
    ...
    const tipoGrafico = document.getElementById('biTipoGrafico').value;
    const resumoDiv = document.getElementById('biResumo');

    // Salva a preferência do usuário para sempre
    localStorage.setItem(`pref_grafico_${contextoAtualBI}`, tipoGrafico);

    if (!inicio || !fim) return alert("Selecione as datas!");

    // --- AGREGAR DADOS NO INTERVALO ---
    let labels = [];
    let valores = [];
    let cores = [];
    let textoResumo = "";
    ...
    if (contextoAtualBI === 'financeiro') {
        // Lógica Financeira (Igual ao Dashboard, mas com filtro de data custom)
        let totSalario = 0, totComissao = 0, totMoto = 0, totLoja = 0;

        // Soma Pagamentos
        window.db.pagamentos.forEach(p => { if (p.data >= inicio && p.data <= fim) totSalario += p.valor; });

        // Soma Extras
        window.db.extras.forEach(e => {
            if (e.data >= inicio && e.data <= fim) {
                if (e.tipo === 'Despesa') totLoja += e.valor;
                else if (e.tipo === 'Comissao') totComissao += e.valor;
            }
        });

        // Soma Motoboys
        if (window.db.entregas) {
            window.db.entregas.forEach(e => { if (e.data >= inicio && e.data <= fim) totMoto += e.valorTotal; });
        }

        labels = ['Salários Pagos', 'Comissões', 'Motoboys', 'Despesas Loja'];
        valores = [totSalario, totComissao, totMoto, totLoja];
        cores = ['#27ae60', '#8e44ad', '#d35400', '#c0392b'];
        
        const totalzao = totSalario + totComissao + totMoto + totLoja;
        textoResumo = `<span style="color:var(--text-main)">Gasto Total no Período: ${fmtMoeda(totalzao)}</span>`;

    } else if (contextoAtualBI === 'vendas') {
        // Lógica de Vendas (Ranking por Vendedor)
        let ranking = {};
        let totalVendasPeriodo = 0;

        window.db.extras.forEach(e => {
            if (e.data >= inicio && e.data <= fim && e.tipo === 'Comissao') {
                // Engenharia reversa: Se a comissão é 7% ou 10%, tentamos achar o valor base na obs ou calcular
                // Para simplificar e ser exato, vamos usar a regra inversa padrão (valor / 0.07) 
                // ou melhor: extrair da OBS se possível, mas o cálculo aproximado serve para o gráfico
                const valorVendaEstimado = e.valor / 0.07; 
                
                if (!ranking[e.beneficiario]) ranking[e.beneficiario] = 0;
                ranking[e.beneficiario] += valorVendaEstimado;
                totalVendasPeriodo += valorVendaEstimado;
            }
        });
        ...
        valores = sorted.map(i => i[1]);
        // Gera cores aleatórias douradas/amarelas
        cores = valores.map(() => '#f1c40f'); 

        textoResumo = `<span style="color:#f39c12">Total Vendido no Período: ${fmtMoeda(totalVendasPeriodo)}</span>`;
    }

    resumoDiv.innerHTML = textoResumo;
    renderizarChartBI(labels, valores, cores, tipoGrafico);
}

// 3. Renderiza o Gráfico no Canvas Grande
function renderizarChartBI(labels, data, colors, type) {
    const ctx = document.getElementById('canvasGraficozao').getContext('2d');
    
    if (chartExpandido) chartExpandido.destroy(); // Destroi o anterior para não sobrepor
            ...
            tooltip: {
                callbacks: {
                    label: function(context) {
                        ...
                        else if (context.parsed !== null) label += fmtMoeda(context.parsed);
                        return label;
                    }
                }
            }
    ...
    // Se for barra ou linha, precisa de eixos
    if (type === 'bar' || type === 'line') {
        options.scales = {
            y: { beginAtZero: true, ticks: { callback: function(value) { return value.toLocaleString('pt-BR', {style:'currency', currency:'BRL'}); } } }
        };
    }
...
// 4. Atalho para mudar tipo sem clicar em filtrar
window.mudarTipoGrafico = function() {
